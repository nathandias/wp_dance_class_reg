<?php
/*
* Template Name: ClassRegistrationHidden
*/
...
<p><small style="color:blue;">Please report any issues you encounter with online registration to <a href="mailto:admin@thisdomain.com">our webmaster.</a></small></p>
...
<form method="post" action="/register-special-group/">
<h3>Student</h3>
<table>
<tbody>
...
<?php
	$class_types = array('Weekly Group', 'Workshop');
	foreach ($class_types as $class_type) {
		
		$classes = $crdb->get_results($crdb->prepare("SELECT * FROM classes WHERE class_type = %s AND ((CURRENT_DATE >= reg_open_date AND CURRENT_DATE <= reg_close_date) AND active='yes' AND tag LIKE '%%%s%%') ORDER BY upstairs_floor ASC, start_date ASC, start_time ASC", $class_type, $valid_tag));
...
?>
							<p style="color:red; font-size:10px; line-height:1.15;">Follower online registration on hold (until leaders catch up).
							Leaders may still sign up online. Drop-in spots <em>may</em> be available at the door for both roles.
							Followers who bring a leader to enroll with them at the door are most likley to get in.</p>
<?php
					}
					if ($class->temp_hold_leaders == 1) {
?>
							<p style="color:red; font-size:10px; line-height:1.15;">Leader online registration on hold (until followers catch up). 
							Followers may still sign up online.  Drop-in spots <em>may</em> be available at the door for both roles.
							Leaders who bring a follower to enroll with them at the door are most likely to get in.</p>
<?php	
					}
...
<strong>NOTE:</strong> Cat's Corner usually takes place at Swedish American Hall except for a few Wednesdays note here, due to private events:
